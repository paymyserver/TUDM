...
import com.android.cloudcovermusic.model.LoginResponse;
...
/**
 * {@link CCMLogin} is the decision making activity of the
 * {@link CCMApplication}.<br>
 ...
 * Once successfully logged in, it redirects user to {@link CCMPlayer} activity
 * which plays the tracks/songs once the player is activated.
 * 
 * @author Mahesh Chauhan
 * 
 */
public class CCMLogin extends Activity {
    private static final String TAG = CCMLogin.class.getSimpleName();
 ...
 // add custom font for text view
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(mAuthToken) && CCMApplication.sIsSessionOngoing) {
        ...
        setContentView(R.layout.ccm_login_screen_layout);
        ...
        mSignIn.setOnClickListener(new View.OnClickListener() {
            ...
            /*
                ...
                if (TextUtils.isEmpty(username) || TextUtils.isEmpty(password)) {
                	...
                	if(CCMUtils.isDevelopmentAppVersion(getApplicationContext()) || CCMUtils.checkDebugFile())
                	{
                	...
                	if(ServerUtilities.isNetworkAvailable(CCMLogin.this))
                	{
                    loginUser();
                	}else{
    ...
    /**
     * Launches the {@link CCMSettingActivity} screen which let developer/user
     * to change some developer settings.
     */
    void launchSettingsActivity() {
        Intent launchNext = new Intent(getApplicationContext(),
                CCMSettingActivity.class);
        startActivity(launchNext);
    }

    /**
     * Launches the {@link CCMPlayer} screen which will start playing the track
     * if the player is already activated.<br>
     * Else will let user to activate the player.
     * 
     * @param firstTime
     *            Suggests whether the {@link CCMPlayer} is launching first time
     *            after login.
     */
    void launchMainActivity(boolean firstTime, boolean forceActivation) {
        Intent launchNext = new Intent(getApplicationContext(), CCMPlayer.class);
    ...
    class SignInTaskAsync extends AsyncTask<Void, String, String> {
        ...
        ProgressDialog asyncDialog = new ProgressDialog(CCMLogin.this);
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                            ...
                            if (registerDeviceResponse.isSuccess()) {
                                ...
                                if (errorResponse != null
                                        ...
                                        && errorResponse
                                                .getCode()
                                                .equalsIgnoreCase(
                                                        CCMErrorHandler.ERROR_DUPLICATE_DEVICE)) {
                                    Editor editor = mPref.edit();
                                    editor.putString(
                                            Constants.PARAM_DEVICE_AUTH_TOKEN,
                                            mAuthToken);
                                    ...
                                    // Its a failure.
                                	if(CCMUtils.isDevelopmentAppVersion(getApplicationContext()))
                                	{
                ...
                if(CCMUtils.isDevelopmentAppVersion(getApplicationContext()))
            	{
                if (exceptionType == CCMException.IO_EXCEPTION) {
                    sRet = CCMErrorHandler.getErrorMessage(context,
                            exceptionType);
                }
            	}
                e.printStackTrace();
        ...
        protected void onPostExecute(String errorMsg) {
            ...
            if (errorMsg == null) {
                ...
                CCMApplication.sIsSessionOngoing = true;
    ...
    protected ResponseData sendSignInRequest(String username, String password)
        ...
        nameValuePairs.add(new BasicNameValuePair(Constants.PARAM_DEVICE_UUID,
                CCMUtils.getUUID(getApplicationContext(), username)));
    ...
    protected ResponseData sendRegisterDeviceRequest(String token, int custId,
            int userId, String username) throws CCMException {
        ...
        nameValuePairs.add(new BasicNameValuePair(Constants.PARAM_DESCRIPTION,
                ""+CCMUtils.getDeviceAndSdkVersion(CCMLogin.this)
                +" Resolution "+CCMUtils.getDeviceResolution(CCMLogin.this)));
        nameValuePairs.add(new BasicNameValuePair(Constants.PARAM_DEVICE_NAME,
                CCMUtils.getDeviceName()));
        ...
        nameValuePairs.add(new BasicNameValuePair(Constants.PARAM_UUID,
                CCMUtils.getUUID(context,username)));
